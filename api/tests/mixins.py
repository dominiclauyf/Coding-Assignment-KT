...
from oauth.tests.factories import AccessTokenFactory
...
class APITestCase(NotLoggedInAPITestCase):
    def setUp(self):
        super().setUp()
        self.token = AccessTokenFactory()

    def assertResponseNotAuthenticated(self, url, method=HTTPMethod.GET, **kwargs):
        super(APITestCase, self).assertResponseStatusCode(
            url, method, status.HTTP_401_UNAUTHORIZED, **kwargs
        )

    def assertResponseStatusCode(
        self, url, method=HTTPMethod.GET, status_code=status.HTTP_200_OK, **kwargs
    ):
        self.client.credentials(
            self.token.user, HTTP_AUTHORIZATION=f"Bearer {self.token.token}"
        )
        return super().assertResponseStatusCode(
            url, method=method, status_code=status_code, **kwargs
        )
    ...
    def assertBadRequestResponse(
        ...
        Run request after login.
        """
        self.client.credentials(
            self.token.user, HTTP_AUTHORIZATION=f"Bearer {self.token.token}"
        )
        super().assertBadRequestResponse(
            url, method, data, error_field=error_field, error_msg=error_msg, debug=debug
        )
    ...
    def run_request_instance_not_found(self, urls, method):
        ...
        super().run_request_instance_not_found(urls, method)
